...
export default function Home() {
  ...
  const [isHomeIndustryModalOpen, setIsHomeIndustryModalOpen] = useState(false);

  // Industry options for Home Search modal (matching Brand Listing design)
  const homeIndustryOptions: FilterOption[] = useMemo(() => {
    const baseList: { id: string; label: string }[] = [
      ...
      { id: 'Automobile & EV', label: 'Automobile & EV' },
      { id: 'Beauty & Salon', label: 'Beauty & Salon' },
      { id: 'Other', label: 'Other' },
    ];

    const result: FilterOption[] = [...baseList];

    (brands || []).forEach((b) => {
      if (b.industry && b.industry.trim()) {
        const indTrim = b.industry.trim();
        const exists = result.some(
          (o) => o.id.toLowerCase() === indTrim.toLowerCase() || o.label.toLowerCase() === indTrim.toLowerCase()
        );
        if (!exists) {
          result.push({ id: indTrim, label: indTrim });
        }
      }
    });
  ...
  const currentHomeIndustryLabel = useMemo(() => {
    if (!homeSearchIndustry || homeSearchIndustry.toUpperCase() === 'ALL') return 'All Industries';
    const found = homeIndustryOptions.find(
      (o) => o.id.toLowerCase() === homeSearchIndustry.toLowerCase() || o.label.toLowerCase() === homeSearchIndustry.toLowerCase()
    );
    return found ? found.label : homeSearchIndustry;
  }, [homeSearchIndustry, homeIndustryOptions]);
  ...
  const handlePerformSearch = () => {
    ...
    const dest = `/brands${params.toString() ? `?${params.toString()}` : ''}`;
    if (!isAuthenticated) {
      setAuthModalConfig({
        isOpen: true,
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#F5F9FF] text-[#0F172A] font-sans selection:bg-blue-200 selection:text-blue-900">
      ...
      {/* SECTION 1: HERO CONTAINER (Full Width, Responsively Scaled for All Breakpoints) */}
      <section className="relative w-full overflow-hidden bg-slate-900 text-[#0F172A] flex flex-col justify-center min-h-[500px] sm:min-h-[560px] md:min-h-[600px] lg:min-h-[640px] xl:min-h-[680px] py-16 sm:py-20 lg:py-24">
        ...
        <div className="hidden sm:flex absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 flex-col items-center gap-1 text-slate-400 text-[10px] font-black uppercase tracking-widest pointer-events-none z-10">
          <span>Scroll</span>
          <div className="w-4 h-7 border border-slate-300/80 rounded-full flex justify-center p-1 shadow-xs">
        ...
        <div className="bg-gradient-to-br from-white via-blue-50/40 to-slate-50 border border-blue-100/80 rounded-[32px] p-6 sm:p-10 shadow-[0_20px_50px_rgba(15,23,42,0.04)] relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-3 sm:gap-4 bg-white p-3 sm:p-4 rounded-2xl border border-slate-200 shadow-lg">
              ...
              {/* Industry Select Option with matching bottom-sheet design */}
              <div className="md:col-span-4 relative flex items-center">
                <button
                  type="button"
                  onClick={() => setIsHomeIndustryModalOpen(true)}
                  className={`w-full flex items-center justify-between bg-slate-50/80 border hover:border-blue-400 hover:bg-slate-100 text-xs sm:text-sm font-semibold rounded-xl px-4 py-3 outline-none transition-all cursor-pointer ${
                    homeSearchIndustry && homeSearchIndustry.toUpperCase() !== 'ALL'
                      ? 'border-blue-500 bg-blue-50/60 text-blue-900 shadow-xs'
                      : 'border-slate-200 text-slate-700'
                  }`}
                >
                  <div className="flex items-center gap-2.5 min-w-0 pr-2">
                    <Filter size={15} className="text-blue-600 shrink-0" />
                    <span className="text-slate-400 font-medium">Industry:</span>
                    <span className="font-extrabold text-blue-950 truncate">
                      {currentHomeIndustryLabel}
                    </span>
                  </div>
                  <ChevronDown
                    size={16}
                    className={`text-slate-400 shrink-0 transition-transform duration-200 ${
                      isHomeIndustryModalOpen ? 'rotate-180 text-blue-600' : ''
                    }`}
                  />
                </button>

                <FilterBottomSheet
                  isOpen={isHomeIndustryModalOpen}
                  onClose={() => setIsHomeIndustryModalOpen(false)}
                  title="INDUSTRY"
                  icon={<Filter size={16} className="text-blue-600" />}
                  options={homeIndustryOptions}
                  selectedId={homeSearchIndustry}
                  onSelect={(newId) => {
                    setHomeSearchIndustry(newId);
                  }}
                  onReset={() => setHomeSearchIndustry('All')}
              ...
              {[
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const dest = `/brands?industry=${encodeURIComponent(chip.industry)}`;
                    if (!isAuthenticated) {
                      setAuthModalConfig({
                        isOpen: true,
                        action: 'browse_brands',
      ...
      <section className="pt-6 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        ...
        {!isAuthenticated ? (
          <div className="bg-white border border-blue-100 rounded-3xl p-8 sm:p-12 text-center shadow-xl shadow-blue-900/5">
            <div className="max-w-xl mx-auto space-y-4">
              ...
              <p className="text-slate-600 text-sm sm:text-base leading-relaxed">
                Create your Brix India account to explore verified franchise brands, compare opportunities and connect with brands.
              </p>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-4">
                <Link
                  to="/register?role=FRANCHISE_SEEKER&redirect=%2Fbrands"
                  className="w-full sm:w-auto px-8 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-lg shadow-blue-600/25 transition-all text-center"
                >
                  Sign Up
                </Link>
                <Link
                  to="/login?redirect=%2Fbrands"
                  className="w-full sm:w-auto px-8 py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-black text-xs uppercase tracking-wider rounded-xl transition-all text-center"
                >
                  Login
                </Link>
              </div>
              <p className="text-xs text-slate-400 pt-2">
                ...
                <Link to="/login?redirect=%2Fbrands" className="text-blue-600 font-bold hover:underline">
                  Login
                </Link>
              </p>
            </div>
      ...
      <section className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 lg:gap-6">
          ...
          {/* Stage 1 Card */}
          <div className="bg-white border border-[#DCE7F5] rounded-2xl sm:rounded-[24px] p-5 sm:p-6 shadow-xs hover:shadow-[0_12px_32px_-8px_rgba(37,99,235,0.12)] hover:-translate-y-1 transition-all duration-300 text-left flex flex-col sm:flex-row md:flex-col lg:flex-row items-start sm:items-center md:items-start lg:items-center justify-between gap-4 overflow-hidden group">
            ...
            <div className="w-24 h-24 sm:w-28 sm:h-28 md:w-full md:h-32 lg:w-28 lg:h-28 xl:w-32 xl:h-32 shrink-0 self-center sm:self-auto md:self-center lg:self-auto">
              <VerificationVisual />
            </div>
          </div>

          {/* Stage 2 Card */}
          <div className="bg-blue-50/50 border border-blue-100/80 rounded-2xl sm:rounded-[24px] p-5 sm:p-6 shadow-xs hover:shadow-[0_12px_32px_-8px_rgba(37,99,235,0.12)] hover:-translate-y-1 transition-all duration-300 text-left flex flex-col sm:flex-row md:flex-col lg:flex-row items-start sm:items-center md:items-start lg:items-center justify-between gap-4 overflow-hidden group">
            <div className="min-w-0 flex-1 flex flex-col justify-center">
              <div className="flex items-center gap-2 mb-2">
      ...
      <section className="py-16 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full bg-blue-50/50 rounded-[32px] sm:rounded-[40px] border border-blue-100/50 my-10 sm:my-14">
        <div className="text-center max-w-2xl mx-auto mb-10 sm:mb-14">
          <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-blue-50 text-blue-700 text-xs font-black uppercase tracking-wider rounded-full border border-blue-100/50 mb-4 sm:mb-6">Industry Demographics</span>
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black text-[#0F172A] uppercase leading-tight">High-Growth Sectors in India</h2>
          <p className="text-[#64748B] font-semibold text-xs sm:text-sm mt-3 border-t border-slate-100 pt-3">Discover verified opportunities sorted by highest trending CAGR market yield indices.</p>
        </div>
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full bg-blue-50/40 rounded-[28px] border border-blue-100/60 my-8 sm:my-12 flex flex-col justify-center overflow-hidden">
        <div className="text-center max-w-3xl mx-auto mb-8 sm:mb-10">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 text-blue-700 text-xs font-black uppercase tracking-wider rounded-full border border-blue-100 mb-3">Success Covenants</span>
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black text-[#0F172A] uppercase tracking-tight leading-tight">Vetted Expansion Case Covenants</h2>
          <p className="text-[#64748B] font-semibold text-xs sm:text-sm mt-2 border-t border-slate-100/80 pt-2">Read how brands launched sustainable outlets with zero broker fees.</p>
        </div>
        ...
        <div className="relative flex overflow-x-hidden">
          ...
          >
            {[
              ...
              <div key={idx} className="flex-shrink-0 w-[260px] sm:w-[300px] md:w-[340px] bg-white p-5 rounded-2xl border border-[#DCE7F5] shadow-xs flex flex-col justify-between relative">
                <div className="absolute top-2 right-3 text-4xl text-blue-600 font-serif leading-none opacity-10">"</div>
                <div className="space-y-2.5 relative z-10">
                  ...
                  <p className="text-xs sm:text-[13px] text-slate-600 font-semibold leading-relaxed border-l-2 border-blue-500 pl-2.5">
                    "{testimonial.text}"
                  </p>
                </div>
                <div className="border-t border-slate-100 pt-3 mt-4">
                  <div className="font-extrabold text-[#0F172A] text-xs uppercase tracking-tight">{testimonial.name}</div>
                  <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{testimonial.role}</div>
                </div>
              </div>
            ))}
          </motion.div>
