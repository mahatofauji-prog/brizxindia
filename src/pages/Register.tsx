...
export default function Register() {
  ...
  const initialRole: 'brand' | 'seeker' = paramRole === 'brand' || paramRole === 'BRAND_OWNER' ? 'brand' : 'seeker';
  ...
  // Role & Email State
  const [role, setRole] = useState<'brand' | 'seeker'>(initialRole);
            ...
            {!isOtpSent ? (
              <form onSubmit={handleSendOtp} className="space-y-6">
                ...
                <p className="text-xs text-slate-500 pt-2">
                  Already have an account? <Link to={redirectTo ? `/login?redirect=${encodeURIComponent(redirectTo)}` : '/login'} className="text-blue-600 font-bold hover:underline">Sign In</Link>
                </p>
              </form>
            ) : (
