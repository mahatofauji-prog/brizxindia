        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative flex-1 min-w-[260px]">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-2xl pl-11 pr-4 py-2.5 text-xs font-bold text-slate-800 outline-none focus:border-blue-500 focus:bg-white transition-all"
            />
          </div>
          ...
          {/* City Filter */}
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl px-3 py-2 text-xs font-bold text-slate-700">
            <MapPin size={14} className="text-blue-600 shrink-0" />
            <span className="text-slate-500 font-medium">City:</span>
            <select
              value={selectedCity}
              onChange={(e) => setSelectedCity(e.target.value)}
              className="bg-transparent text-blue-800 font-extrabold outline-none cursor-pointer"
            >
              {cities.map(c => (
          ...
          <SortFilter
            ...
            allowedSortKeys={['MATCH', 'INVESTMENT_LOW', 'INVESTMENT_HIGH', 'OUTLETS']}
          />
        </div>

        {/* Active Filters Summary */}
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-blue-50">
          <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
            <span>Filters:</span>
            <span className="text-blue-700 font-extrabold bg-blue-50 px-2 py-0.5 rounded-lg">
              {selectedIndustry === 'ALL' ? 'All Industries' : selectedIndustry}
            </span>
            <span className="text-slate-300">•</span>
            <span className="text-blue-700 font-extrabold bg-blue-50 px-2 py-0.5 rounded-lg">
              {maxInvestment === null ? 'Any Budget' : `≤ ₹${maxInvestment}L`}
            </span>
            {selectedCity !== 'ALL' && (
              <>
                <span className="text-slate-300">•</span>
