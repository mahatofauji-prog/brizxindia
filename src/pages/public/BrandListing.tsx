...
export default function BrandListing({ initialOpenSmartMatch }: { initialOpenSmartMatch?: boolean }) {
  const { brands } = useData();
  const location = useLocation();
  const navigate = useNavigate();
  const [smartMatchOpen, setSmartMatchOpen] = useState<boolean | undefined>(
    initialOpenSmartMatch || location.search.includes('smartmatch=true') || location.pathname.includes('smart-match') ? true : undefined
  );
  ...
  // Dynamic list of unique industries from brands
  const availableIndustries = useMemo(() => {
    const list = new Set<string>();
    brands.forEach((b) => {
      if (b.industry && b.industry.trim()) {
        list.add(b.industry.trim());
      }
    });
  ...
  const filteredBrands = brands.filter((brand) => {
    ...
    const cleanSel = (selectedIndustry || 'ALL').trim().toUpperCase();
    const cleanInd = (industry || '').trim().toUpperCase();
    const matchesIndustry =
      cleanSel === 'ALL' ||
      cleanInd === cleanSel ||
      cleanInd.includes(cleanSel) ||
      cleanSel.includes(cleanInd) ||
      (cleanSel === 'OTHER' && !['FOOD', 'HEALTH', 'EDU', 'RETAIL', 'FITNESS', 'AUTOMATION', 'AUTOMOBILE', 'BEAUTY'].some(c => cleanInd.includes(c)));
