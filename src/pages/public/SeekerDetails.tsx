...
import { getSeekerAvatarUrl, getSeekerInitials } from '../../utils/seekerAvatar';
import { 
  ShieldCheck, MapPin, Briefcase, IndianRupee, Clock, ArrowLeft,
  ChevronRight, Phone, Mail, Lock, Sparkles, User, CheckCircle2,
  Building2, ArrowRight, Bookmark, Unlock, Calendar as CalendarIcon, AlertCircle
} from 'lucide-react';
...
export default function SeekerDetails() {
  ...
  const [authModalAction, setAuthModalAction] = useState<'unlock_seeker' | 'save_lead' | 'crm_access' | 'schedule_meeting'>('unlock_seeker');
  ...
  const avatarUrl = getSeekerAvatarUrl(seeker);
  const initials = getSeekerInitials(seekerName);
  ...
  const handleSaveLead = () => {
    if (!user) {
      setAuthModalAction('save_lead');
      setShowAuthModal(true);
      return;
    }
    if (user.role !== 'BRAND_OWNER') {
      setRoleWarning('Lead pipeline and bookmarking tools are designed for registered Brand Owner accounts.');
      return;
    }
    if (currentBrand) {
      toggleSaveLeadForBrand(currentBrand.id, seeker.id);
    }
  };

  const handleOpenCrm = () => {
    if (!user) {
      setAuthModalAction('crm_access');
      setShowAuthModal(true);
      return;
    }
    if (user.role !== 'BRAND_OWNER') {
      setRoleWarning('BrizX CRM pipeline is designed for registered Brand Owner accounts.');
      return;
    }
    navigate('/brand/crm');
  };
  ...
  const handleUnlockClick = () => {
    if (!user) {
      setAuthModalAction('unlock_seeker');
      setShowAuthModal(true);
      return;
    }
    if (user.role !== 'BRAND_OWNER') {
      setRoleWarning('Direct investor contact unlocking is reserved for registered Brand Owner accounts.');
      return;
    }
  ...
  const handleScheduleMeetingClick = () => {
    if (!user) {
      setAuthModalAction('schedule_meeting');
      setShowAuthModal(true);
      return;
    }
    if (user.role !== 'BRAND_OWNER') {
      setRoleWarning('Direct pitch meetings are scheduled through Brand Owner accounts.');
      return;
    }
    navigate('/brand/meetings');
  };
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-5">
            <div className="w-20 h-20 bg-blue-50 border-2 border-blue-200 text-blue-700 font-black text-3xl flex items-center justify-center rounded-2xl shrink-0 shadow-xs overflow-hidden font-heading">
              {avatarUrl ? (
                <img src={avatarUrl} alt={seekerName} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
              ) : (
                initials
              )}
            </div>
            ...
            <div className="bg-white rounded-3xl border border-blue-100 p-6 shadow-sm sticky top-28 space-y-4">
              ...
              {roleWarning && (
                <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-2.5 text-amber-900 text-xs">
                  ...
                  <p className="font-medium leading-relaxed">{roleWarning}</p>
                </div>
              )}
              ...
              >
                <Bookmark size={16} /> {isSaved ? 'Lead Saved' : 'Save Lead'} {!user && <Lock size={12} className="text-slate-400" />}
              </button>

              <button
                type="button"
                onClick={handleOpenCrm}
                className="w-full py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-2xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-sm cursor-pointer"
              >
                Open CRM {!user && <Lock size={12} className="text-slate-400" />} <ArrowRight size={14} />
              </button>
              ...
              {isUnlocked ? (
                <div className="space-y-3 pt-2">
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-md shadow-blue-100 cursor-pointer"
                  >
                    <CalendarIcon size={14} /> Schedule Meeting {!user && <Lock size={12} className="text-blue-200" />}
                  </button>
                </div>
              ) : (
                <button
                ...
                >
                  <Unlock size={16} /> Unlock Contact {!user ? '(Login Required)' : '(1 Credit)'} {!user && <Lock size={12} className="text-blue-200" />}
                </button>
              )}
