...
export default function BrandDetails() {
  ...
  // Auth & Connection Modal states
  const [showAuthModal, setShowAuthModal] = useState(false);
  const [authModalAction, setAuthModalAction] = useState<'submit_application' | 'contact_brand'>('submit_application');
  const [showBrandConnectionModal, setShowBrandConnectionModal] = useState(false);
  ...
  const expectedOutletCountStr = brand?.totalOutlets || (brand as any)?.outlets ? `${brand?.totalOutlets || (brand as any)?.outlets}+ Active Outlets` : getEnrichedBrandData(brand).investmentOverview.expectedOutletCount;
  const businessModelTypeStr = brand?.businessModel || getEnrichedBrandData(brand).investmentOverview.businessModelType;
  const establishedYearStr = brand?.establishedYear || (brand as any)?.established ? String(brand?.establishedYear || (brand as any)?.established) : getEnrichedBrandData(brand).investmentOverview.establishedYear;
  const industryStr = brand?.industry || getEnrichedBrandData(brand).investmentOverview.industry;

  // 1. Restore persisted form data from sessionStorage if previously filled
  useEffect(() => {
    if (!brand) return;
    try {
      const saved = sessionStorage.getItem(`brizx_pending_app_${brand.id}`);
      if (saved) {
        const parsed = JSON.parse(saved);
        setFormData(prev => ({
          ...prev,
          ...parsed,
          investmentBudget: parsed.investmentBudget || prev.investmentBudget || investmentRequiredStr
        }));
      } else {
        setFormData(prev => ({
          ...prev,
  ...
  // 2. Prefill authenticated user profile details if form fields are empty
  useEffect(() => {
    if (user) {
      setFormData(prev => ({
        ...prev,
        ...
        mobile: prev.mobile || (user as any).seekerData?.phone || '',
        city: prev.city || (user as any).seekerData?.city || '',
        state: prev.state || (user as any).seekerData?.state || ''
      }));
    }
  }, [user]);

  // 3. Auto-submit pending application if user returns authenticated as Seeker
  useEffect(() => {
    if (!brand || !user) return;
    const hasPendingSubmit = sessionStorage.getItem(`brizx_pending_submit_${brand.id}`) === 'true';
    if (hasPendingSubmit && user.role === 'FRANCHISE_SEEKER') {
      sessionStorage.removeItem(`brizx_pending_submit_${brand.id}`);
      sessionStorage.removeItem(`brizx_pending_app_${brand.id}`);
      
      addApplication({
        brandId: String(brand.id),
        brandName: brand.brandName || 'Franchise Partner',
        seekerId: user.id,
        applicantName: formData.applicantName || user.name || 'Applicant',
        mobile: formData.mobile || (user as any).seekerData?.phone || '+91 9876543210',
        email: formData.email || user.email || '',
        whatsApp: formData.whatsApp || formData.mobile || '',
        city: formData.city || 'India',
        state: formData.state || '',
        investmentBudget: formData.investmentBudget || investmentRequiredStr,
        availableCapital: formData.availableCapital,
        preferredLocation: formData.preferredLocation || 'Main Catchment Area',
  ...
  // Helper to update form fields and persist directly to sessionStorage
  const handleInputChange = (field: keyof typeof formData, value: any) => {
    setFormData(prev => {
      const updated = { ...prev, [field]: value };
      if (brand) {
        try {
          sessionStorage.setItem(`brizx_pending_app_${brand.id}`, JSON.stringify(updated));
        } catch (e) {
          // Ignore quota error
        }
      }
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    // GUEST RESTRICTION: Require authentication to submit application
    if (!user) {
      try {
        sessionStorage.setItem(`brizx_pending_app_${brand.id}`, JSON.stringify(formData));
        sessionStorage.setItem(`brizx_pending_submit_${brand.id}`, 'true');
      } catch (err) {
        console.error('Session storage error:', err);
      }
      setAuthModalAction('submit_application');
      setShowAuthModal(true);
      return;
    }

    // Role check: Only Franchise Seekers submit applications
    if (user.role === 'BRAND_OWNER') {
      setRoleNotice('You are currently signed in with a Brand Owner account. Franchise applications must be submitted by Franchise Seekers. Please switch to or register a Seeker account to submit.');
      return;
    }
  ...
  // Direct Contact Brand click handler
  const handleContactBrandClick = () => {
    if (!user) {
      setAuthModalAction('contact_brand');
      setShowAuthModal(true);
      return;
    }
    if (user.role === 'FRANCHISE_SEEKER') {
      setShowBrandConnectionModal(true);
    } else {
      setRoleNotice('Direct inquiry messaging and connection requests are designed for Franchise Seekers.');
    }
  };
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-800 pb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-7 space-y-5">
              ...
              <div className="pt-2 flex flex-wrap items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={handleContactBrandClick}
                  className="inline-flex items-center gap-2 px-6 py-3.5 bg-white hover:bg-slate-50 text-slate-800 border border-slate-300 hover:border-slate-400 rounded-xl text-sm font-black uppercase tracking-wider transition-all shadow-xs cursor-pointer"
                >
                  <Mail size={16} className="text-blue-600" />
                  <span>Contact Brand</span>
                  {!user && <Lock size={13} className="text-slate-400 ml-0.5" />}
                </button>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
              ...
              <form onSubmit={handleFormSubmit} className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-5">
                ...
                {roleNotice && (
                  <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3 text-amber-900 text-xs">
                    ...
                    <div>
                      <p className="font-bold">Account Action Required</p>
                      <p className="mt-0.5 text-amber-800">{roleNotice}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => handleInputChange('franchiseType', e.target.value)}
                ...
                <div>
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      <>
                        <Send size={16} /> SUBMIT APPLICATION <Lock size={14} className="opacity-90 ml-0.5" />
                      </>
                    ) : (
                      <>
                  ...
                  {!user && (
                    <p className="text-center text-[11px] text-slate-500 font-medium mt-2 flex items-center justify-center gap-1.5">
                      <Lock size={12} className="text-blue-600" />
                      Sign in required upon submission. Your filled details will be safely preserved.
                    </p>
                  )}
                </div>
        ...
        {/* AUTH REQUIRED MODAL FOR GUEST INTERACTIONS */}
        <AuthRequiredModal
        ...
        {/* DIRECT BRAND CONNECTION / INQUIRY MODAL */}
        {showBrandConnectionModal && (
          <BrandConnectionModal
            brand={brand}
            isOpen={showBrandConnectionModal}
            onClose={() => setShowBrandConnectionModal(false)}
          />
        )}
