  ...
  const industries = useMemo(() => {
    return Array.from(
      new Set(
        brands
          .map(b => b?.industry)
          .filter((ind): ind is string => typeof ind === 'string' && ind.trim().length > 0)
      )
    ).sort();
  }, [brands]);
          ...
          <div className="bg-white rounded-2xl p-4 shadow-xs border border-slate-200 mb-6 flex flex-col md:flex-row gap-4 justify-between items-center shrink-0">
            ...
            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
              <div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200">
                ...
                >
                  <option key="ALL" value="ALL">All Industries</option>
                  {industries.map(ind => (
                    <option key={`industry-${ind}`} value={ind}>{ind}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200">
                ...
                <select 
                  ...
                  className="bg-transparent text-xs font-bold text-blue-700 outline-none cursor-pointer"
                >
                  <option key="ALL" value="ALL">All Plans</option>
