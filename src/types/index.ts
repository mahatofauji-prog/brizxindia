...
export interface BrandFinancialProfile {
  // Capex Breakdown (in ₹ Lakhs)
  franchiseFee?: number;            // One-time Franchise Fee (₹ Lakhs)
  setupCost?: number;               // Setup & Civil Construction Cost (₹ Lakhs)
  interiorCost?: number;            // Interior & Fit-out Cost (₹ Lakhs)
  equipmentCost?: number;           // Kitchen / Machinery / Equipment (₹ Lakhs)
  technologyCost?: number;          // Technology / POS / Signage (₹ Lakhs)
  securityDeposit?: number;         // Landlord Security Deposit (₹ Lakhs)
  workingCapital?: number;          // Initial Working Capital Reserve (₹ Lakhs)
  otherInitialExpenses?: number;    // Other Initial Expenses & Licenses (₹ Lakhs)
  totalInitialInvestment?: number;  // Total Capex in ₹ Lakhs

  // Monthly Revenue Assumptions
  expectedMonthlyRevenue?: number;  // Expected Monthly Revenue (₹ Lakhs)
  avgCustomerTicket?: number;       // Average Customer Ticket (₹)
  estimatedMonthlyOrders?: number;  // Estimated Monthly Orders count

  // Monthly Operating Expenses (OPEX) (in ₹ Rupees or % for royalty/cogs)
  rent?: number;                    // Store / Outlet Rent (₹ / month)
  employeeSalaries?: number;        // Staff Payroll & Salaries (₹ / month)
  electricityUtilities?: number;    // Electricity & Utilities (₹ / month)
  marketingCost?: number;           // Marketing & Promotions (₹ / month)
  royaltyPercentage?: number;       // Brand Royalty Fee (% of monthly revenue)
  maintenanceCost?: number;         // Store Maintenance & Tech Support (₹ / month)
  otherOperatingExpenses?: number;  // Other Operating Expenses (₹ / month)
  cogsPercentage?: number;          // Cost of Goods Sold / Food Cost % (default e.g. 35%)

  // Metadata / Status
  isComplete?: boolean;             // Flag indicating complete profile provided by brand
  updatedAt?: string;               // ISO Timestamp when brand updated financials
  verifiedByAuditor?: boolean;      // BrizX audit verification badge
  notes?: string;                   // Optional auditor or brand notes
}
...
export interface Brand extends User {
  ...
  // Brand Financial Model & ROI Parameters
