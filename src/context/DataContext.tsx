...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadSavedSeekers = (): FranchiseSeeker[] => {
    const saved = localStorage.getItem('brizx_seekers');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
          const mockMap = new Map<string, FranchiseSeeker>();
          mockSeekers.forEach(ms => mockMap.set(ms.id, ms));
          return parsed.map((s: FranchiseSeeker) => {
            const mock = mockMap.get(s.id);
            return {
              ...mock,
              ...s,
              avatar: s.avatar || mock?.avatar,
              availableCapital: s.availableCapital ?? s.investment ?? mock?.availableCapital,
              minInvestment: s.minInvestment ?? mock?.minInvestment,
              maxInvestment: s.maxInvestment ?? mock?.maxInvestment
            };
          });
        }
      } catch (e) {
        console.warn('Failed to parse brizx_seekers from localStorage:', e);
      }
    }
    return mockSeekers;
  };

  const loadSavedBrands = (): Brand[] => {
    ...
    return loadedBrands.map(b => {
      ...
      const mock = mockBrands.find(m => m.id === b.id);
      return {
        ...b,
        brandOrigin: origin,
        financialProfile: b.financialProfile || mock?.financialProfile,
  ...
  const [seekers, setSeekers] = useState<FranchiseSeeker[]>(() => {
    return loadSavedSeekers();
  });

  const [brands, setBrands] = useState<Brand[]>(() => {
    return loadSavedBrands();
  });

  useEffect(() => {
    if (user) {
      setSeekers(prev => prev.length > 0 ? prev : loadSavedSeekers());
      setBrands(prev => prev.length > 0 ? prev : loadSavedBrands());
    }
  }, [user]);
  ...
  // Firestore Seekers Real-Time Sync (Single Source of Truth)
  useEffect(() => {
    if (!user) return;
    let unsubSeekers: (() => void) | undefined;
    let unsubUsers: (() => void) | undefined;

    async function syncFirestoreSeekers() {
      try {
        const { collection, onSnapshot, query, where } = await import('firebase/firestore');
        const { db } = await import('../lib/firebase');

        // 1. Listen to 'seekers' collection
        unsubSeekers = onSnapshot(collection(db, 'seekers'), (snapshot) => {
          if (!snapshot.empty) {
            const firestoreSeekers: FranchiseSeeker[] = [];
            snapshot.forEach(docSnap => {
              const data = docSnap.data();
              firestoreSeekers.push({
                id: docSnap.id,
            ...
            setSeekers(prev => {
              const map = new Map<string, FranchiseSeeker>();
              prev.forEach(s => map.set(s.id, s));
              firestoreSeekers.forEach(s => {
          ...
          console.warn('Firestore seekers listener warning:', err);
        });

        // 2. Also listen to 'users' collection where role == 'FRANCHISE_SEEKER'
        const usersQ = query(collection(db, 'users'), where('role', '==', 'FRANCHISE_SEEKER'));
        unsubUsers = onSnapshot(usersQ, (snapshot) => {
          if (!snapshot.empty) {
            const firestoreUsersAsSeekers: Partial<FranchiseSeeker>[] = [];
            snapshot.forEach(docSnap => {
              firestoreUsersAsSeekers.push({
                id: docSnap.id,
                ...docSnap.data()
              } as Partial<FranchiseSeeker>);
            });

            setSeekers(prev => {
              const map = new Map<string, FranchiseSeeker>();
              prev.forEach(s => map.set(s.id, s));
              firestoreUsersAsSeekers.forEach(u => {
                if (u.id) {
        ...
        console.warn('Error setting up Firestore seekers listener:', err);
      }
    }

    syncFirestoreSeekers();
    return () => {
      if (unsubSeekers) unsubSeekers();
