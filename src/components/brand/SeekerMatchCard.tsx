    ...
    <div className="bg-white rounded-3xl p-6 sm:p-7 shadow-xs hover:shadow-md border border-slate-200/90 transition-all duration-200 group relative">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {breakdown.totalScore >= 90 && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded-full text-[10px] font-black uppercase">
              <Sparkles size={11} className="text-blue-600" /> Top Match {brand?.brandName ? `for ${brand.brandName}` : ''}
            </span>
          )}
        </div>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
        ...
        <div className="flex items-start sm:items-center gap-4 sm:gap-5 min-w-0 flex-1">
          {/* Seeker Avatar */}
          <div className="w-16 h-16 sm:w-20 sm:h-20 bg-blue-50 text-blue-700 font-black rounded-2xl flex items-center justify-center text-xl border-2 border-blue-200 shrink-0 shadow-xs overflow-hidden">
            {getSeekerAvatarUrl(seeker) ? (
              <img
                src={getSeekerAvatarUrl(seeker)}
                alt={seeker.name}
                className="w-full h-full object-cover"
                referrerPolicy="no-referrer"
              />
            ) : (
              <span>{getSeekerInitials(seeker.name)}</span>
            )}
          </div>
          ...
          <button
            ...
            className={`w-16 h-16 sm:w-20 sm:h-20 shrink-0 rounded-2xl border-2 p-1.5 flex flex-col items-center justify-center transition-transform hover:scale-105 cursor-pointer shadow-sm ${scoreTheme.bg}`}
            title="Click to view 100-point match breakdown"
          >
            <span className="text-xl sm:text-2xl font-black font-heading leading-none">
              {breakdown.totalScore}%
            </span>
            <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-wider mt-1">
              Match
            </span>
          </button>
