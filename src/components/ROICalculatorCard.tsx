...
export function ROICalculatorCard({
  ...
  return (
    <div id="roi_calculator_card" className="bg-white rounded-3xl border border-slate-200 shadow-sm text-left font-sans transition-all duration-300 w-full flex flex-col p-5 sm:p-7 lg:p-8 mb-6 sm:mb-8 lg:mb-12">
      {/* Top Header Panel */}
      <div className="flex-none flex items-center justify-between gap-4 mb-6 sm:mb-8">
        <div className="flex items-center gap-3">
          <div className="p-2.5 sm:p-3 bg-blue-50 text-blue-600 rounded-2xl">
            <Calculator size={22} className="stroke-[2.5]" />
          </div>
          <div>
            <h3 className="text-base sm:text-lg font-extrabold text-[#0F172A] uppercase tracking-tight">ROI Calculator</h3>
            <span className="text-xs font-medium text-slate-500">Instant Unit Economics Estimator</span>
          </div>
        </div>

        <button 
          onClick={handleReset}
          className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 text-xs font-black text-slate-500 hover:text-slate-700 bg-slate-50 hover:bg-slate-100 rounded-xl transition-all uppercase tracking-wider"
        >
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-6 lg:gap-8 xl:gap-10 items-stretch">
        ...
        <div className="flex-1 space-y-5 sm:space-y-6 flex flex-col justify-center">
          ...
          <div className="space-y-2.5">
            <div className="flex justify-between items-center">
              <span className="text-xs sm:text-sm font-bold text-slate-700">Initial Investment</span>
              <div className="flex items-center gap-2">
                <input 
                  type="number"
                  value={Math.round(investment)}
                  onChange={(e) => setInvestment(Math.max(1, Number(e.target.value)))}
                  className="w-16 sm:w-20 text-right text-xs sm:text-sm font-black text-blue-600 bg-slate-50 border border-slate-200 rounded-lg p-1 sm:p-1.5"
                />
                <span className="text-xs sm:text-sm font-bold text-slate-400">Lakhs</span>
              </div>
            </div>
            <input 
              type="range" 
          ...
          <div className="space-y-2.5">
            <div className="flex justify-between items-center">
              <span className="text-xs sm:text-sm font-bold text-slate-700">Expected Monthly Revenue</span>
              <div className="flex items-center gap-2">
                <input 
                  type="number"
                  value={revenue}
                  onChange={(e) => setRevenue(Math.max(0.1, Number(e.target.value)))}
                  className="w-16 sm:w-20 text-right text-xs sm:text-sm font-black text-emerald-600 bg-slate-50 border border-slate-200 rounded-lg p-1 sm:p-1.5"
                />
                <span className="text-xs sm:text-sm font-bold text-slate-400">Lakhs</span>
              </div>
            </div>
        ...
        {/* Right Column: Key Metric Results */}
        <div className="lg:w-[360px] xl:w-[400px] flex flex-col justify-between gap-3 sm:gap-4 shrink-0">
          <div className="bg-gradient-to-br from-blue-50/90 to-sky-50/50 p-4 sm:p-5 rounded-2xl border border-blue-100/80 flex flex-col justify-center">
            <span className="text-[10px] sm:text-xs font-black uppercase text-blue-700 tracking-wider mb-1">Est. Monthly Net Profit</span>
            <div className="text-2xl sm:text-3xl font-black text-emerald-600 tracking-tight">
              ₹{calculations.monthlyProfitRupees.toLocaleString('en-IN')}
            </div>
            <span className="text-[11px] font-medium text-slate-500 mt-1">Based on standard operating margins</span>
          </div>

          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col justify-center">
              <span className="text-[10px] sm:text-xs font-black uppercase text-slate-400 tracking-wider mb-1">Break-Even</span>
            ...
            <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col justify-center">
              <span className="text-[10px] sm:text-xs font-black uppercase text-slate-400 tracking-wider mb-1">Annual ROI</span>
              <div className="text-lg sm:text-xl font-black text-blue-600">
