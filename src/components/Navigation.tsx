...
export function Navigation() {
  ...
  // Auth gating modal state for protected menu items
  const [authModalConfig, setAuthModalConfig] = useState<{
    isOpen: boolean;
    action: 'browse_brands' | 'browse_seekers';
    redirectUrl: string;
  } | null>(null);
  ...
  const primaryNavLinks: NavLinkItem[] = [
    { name: 'Home', path: '/' },
    { name: 'Brands', path: '/brands', requiresAuth: true, action: 'browse_brands' },
    { name: 'Seekers', path: '/seekers', requiresAuth: true, action: 'browse_seekers' },
  ...
  const handleNavLinkClick = (e: React.MouseEvent, link: { name: string; path: string; requiresAuth?: boolean; action?: 'browse_brands' | 'browse_seekers' }) => {
    if (link.requiresAuth && !isAuthenticated) {
      e.preventDefault();
      setAuthModalConfig({
        isOpen: true,
        action: link.action || 'browse_brands',
  ...
  return (
    <header className={`sticky top-0 z-[9999] w-full transition-all duration-300 ${
          ...
          <div className="flex flex-col space-y-2.5 font-bold text-sm text-slate-600">
            {allNavLinks.map((link) => (
              <Link
                ...
                onClick={(e) => {
                  if (link.requiresAuth && !isAuthenticated) {
                    handleNavLinkClick(e, link as any);
                  } else {
                    setMobileMenuOpen(false);
                  }
                }}
      ...
      {/* Auth Modal for protected navigation clicks */}
      {authModalConfig && (
        <AuthRequiredModal
